...
"al2")
    ...
    ami_name_al2_x86=$(aws ec2 describe-images --region "$region" --owner "679593333241" --filters "Name=name,Values=CIS Amazon ECS Optimized Amazon Linux 2 Benchmark - Level 1*" "Name=architecture,Values=x86_64" --query "sort_by(Images, &CreationDate)[-1:].[Name]" --output text)
