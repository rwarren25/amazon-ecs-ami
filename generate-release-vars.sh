...
ami_name_x86=$(aws ec2 describe-images --region "$region" --owner "679593333241" --filters "Name=name,Values=CIS Amazon Linux 2 Kernel 4.14*Level 1*" "Name=architecture,Values=x86_64" --query "sort_by(Images, &CreationDate)[-1:].[Name]" --output text)
